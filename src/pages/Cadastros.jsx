...
import RedirectWhatsapp from "../components/redirectWhatsapp/RedirectWhatsapp";
...
const Cadastros = () => {
  // Função para abrir o modal de WhatsApp ao clicar no telefone
  const handleWhatsappClick = (nome, telefone) => {
    setWhatsappModal({ aberto: true, nome, telefone });
  };
  // Função para fechar o modal de WhatsApp
  const handleCloseWhatsappModal = () => {
    setWhatsappModal({ aberto: false, nome: "", telefone: "" });
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
                            ...
                            <ul className="space-y-1">
                              {row.responsaveis.map((resp, idx) => {
                                ...
                                let cpfResp =
                                  typeof resp === "string"
                                    ? resp
                                    : resp.respCpf || resp.cpf;
                                // Buscar nome do responsável pelo CPF na lista de guardians
                                ...
                                nome = guardian
                                  ? guardian.nome ||
                                    ...
                                    guardian.cpf
                                  : cpfResp;
                                // Buscar parentesco individualizado no vínculo
                                if (
                                  guardian &&
                                  Array.isArray(guardian.vinculos)
                                ) {
                                  const vinc = guardian.vinculos.find(
                                    (v) => v.cpfAtleta === row.cpf,
                                  );
                                  if (vinc) parentesco = vinc.parentesco;
                                }
                ...
                <DataTable
                  columns={[
                        ...
                        <button
                          ...
                          onClick={() => {
                            setItemEditando({
                              ...
                              cpf: row.cpf || "",
                              email: row.email || "",
                              telefone: row.phoneNumber || row.telefone || "",
                              nomeAtleta: "",
                              parentesco: row.parentesco || row.kinship || "",
                              cep: row.cep || "",
                              bairro: row.bairro || "",
                              cidade: row.cidade || "",
                              numero: row.numero || "",
                              ...
                              id: row.id || undefined,
                              // Passa o array de CPFs dos atletas vinculados
                              atletas: Array.isArray(row.atletas)
                                ? row.atletas
                                : row.athletes || [],
                            });
                            setAbrirCadastroResponsavel(true);
                    ...
                    },
                    {
                      ...
                      label: "Atletas",
                      render: (value, row) => {
                        // value: array de nomes, row.atletas: array de CPFs
                        if (
                          Array.isArray(value) &&
                          Array.isArray(row.atletas)
                        ) {
                          return value.map((nome, idx) => {
                            const cpf = row.atletas[idx];
                            return (
                              <span key={cpf}>
                                <button
                                  className="text-blue-600 hover:underline cursor-pointer font-medium"
                                  onClick={() => {
                                    const atleta = players.find(
                                      (p) => p.cpf === cpf,
                                    );
                                    if (atleta) {
                      ...
                      render: (value, row) =>
                        value ? (
                          <span
                            ...
                            onClick={() => handleWhatsappClick(row.name, value)}
                            title={`Entrar em contato com ${row.name} via WhatsApp`}
                          >
                            {formatarTelefoneExibicao(value)}
                          </span>
                        ) : (
                          "—"
                        ),
                  ...
                  data={responsibleFiltrados.map((g) => {
                    // Monta nomes dos atletas vinculados
                    let atletasNomes = [];
                    if (Array.isArray(g.atletas)) {
                      atletasNomes = g.atletas
                        .map((cpf) => {
                          const atleta = players.find((p) => p.cpf === cpf);
                          return atleta ? atleta.name : null;
                        })
                        .filter(Boolean);
                    }
                    return {
                      ...
                      name: g.nome || g.name || "",
                      atletasNomes,
                      parentesco: g.parentesco || g.kinship || "",
                      telefone: g.telefone || g.phoneNumber || "",
                    };
                  })}
                  renderActions={(row) => (
                    <button
                  ...
                  columns={[
                    {
                      ...
                      render: (value, row) => (
                        <button
                          ...
                          className="text-blue-600 hover:underline cursor-pointer font-bold bg-none border-none p-0"
                        >
                          {value}
                        </button>
                      ),
                      ...
                      render: (value, row) =>
                        value || row.phoneNumber ? (
                          <span
                            ...
                            title={`Entrar em contato com ${row.name} via WhatsApp`}
                          >
                            {formatarTelefoneExibicao(value || row.phoneNumber)}
                          </span>
                        ) : (
                          "-"
                        ),
                          ...
                          render: (value, row) =>
                            value ? (
                              <span
                                className="text-blue-600 hover:underline cursor-pointer"
                                onClick={() =>
                                  handleWhatsappClick(row.name, value)
                                }
                                title={`Entrar em contato com ${row.name} via WhatsApp`}
                              >
                                {formatarTelefoneExibicao(value)}
        ...
        <ModalCadastroResponsavel
          ...
          onSave={(data) => {
            (async () => {
              if (itemEditando?.id) {
                try {
                  await update("guardians", itemEditando.id, data);
                  const atualizados = await list("guardians");
                  setGuardians(Array.isArray(atualizados) ? atualizados : []);
                } catch (e) {
                  alert(
                    "Erro ao salvar alterações do responsável: " +
                      (e?.message || e),
                  );
                }
              } else {
                const newId = Math.max(...guardians.map((r) => r.id), 0) + 1;
                setGuardians((prev) => [...prev, { id: newId, ...data }]);
              }
              setItemEditando(null);
              setAbrirCadastroResponsavel(false);
            })();
        ...
        <RedirectWhatsapp
          aberto={whatsappModal.aberto}
          nome={whatsappModal.nome}
          telefone={whatsappModal.telefone}
