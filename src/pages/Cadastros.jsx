...
const Cadastros = () => {
	...
	const trainersComDados = trainers.map((t) => {
		const usuario = usuarios.find((u) => String(u.id) === String(t.user_id));
		return {
			...t,
			name: usuario?.name || usuario?.nome || '-',
      email: usuario?.email || '-',
			phone: usuario?.phone || usuario?.telefone || '-',
		};
	});
	...
	const handleCreated = async (resource, data) => {
		try {
			...
			if (resource === 'trainers')
				setTrainers((prev) => [...prev.filter((t) => t.id !== saved.id), saved]);
	...
	const handleConfirmDelete = async () => {
		...
		try {
			...
			if (recurso === 'players') setPlayers((prev) => prev.filter((item) => item.id !== id));
			if (recurso === 'guardians')
				setGuardians((prev) => prev.filter((item) => item.id !== id));
			if (recurso === 'trainers')
				setTrainers((prev) => prev.filter((item) => item.id !== id));
			if (recurso === 'classes') setClasses((prev) => prev.filter((item) => item.id !== id));
	...
	const handleSalvarUsuario = async () => {
		...
		if (
			...
			alert('Nome, email, data de nascimento, CPF, função e status são obrigatórios');
			return;
		}
	...
	const handleConfirmDeleteUsuario = async () => {
		try {
			...
			setUsuarios((prev) => prev.filter((u) => u.id !== confirmDeleteUsuario.usuarioId));
					...
					<div className="shadow-xl w-full">
                      ...
                      render: (value, row) => (
                          <button
                              ...
                              className="text-blue-600 hover:underline cursor-pointer font-medium text-left bg-none border-none p-0"
                          >
                              {value}
                          </button>
                      ),
				...
				<EditPlayersModal
					...
					onSave={async (data) => {
						try {
							...
							alert('Erro ao salvar alterações do atleta: ' + (e?.message || e));
						}
					}}
				/>
